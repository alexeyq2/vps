...
2. Отредактовать `.env`, переменные:
  - `VPS_DOMAIN` - свой домен
  - `VPS_EMAIL`  - свой email (необязательно, для Let's Encrypt)
  - `VPS_N`       - рандомный номер, он поменяет:
    - `FILEZ_URL`   - уникальный путь до статической веб-шары
    - `SUBSCRIPTION_URL` - уникальный путь (секретный) для подписок (subscription) VPN-клиентов. это должен! быть уникальный URL
  - `ACME_SERVER` - тестовый или реальный сервер Let's Encrypt

3. Создать папку `srv` — тут хранятся конфиги VPN, сертификат и содержимое веб-сайта.
...
* `./log.sh`  # [-f], [-t] - показать время, [container-name - см. compose.yaml]
* пример: `./log.sh -f -t 3x-ui`
...
Потому что nginx слушает на внутреннем порту, трафик до него пробрасывает xray-прокси.
...
2. сменить путь к панели на рандом вроде /vps52050 (важно)
...
2. Запустите `./certificate-delete.sh`
3. Выведется лог получения сертификата


### Окончательная настройка VPN

Лучше заходить на панель по IP-адресу, а не по имени домена, чтобы QR и *ключи сервера создавались по IP-адресу*, а не по доменному имени. Так надежнее.
...
Удалить `srv` и `_work`, скопируйте `srv-default` в `srv` и повторите настройку. Следующий скрипт все сбросит как было (кроме  `.env` - его вы, скорее всего, хотите оставить):

`./reconfig-from-scratch.sh`

# Вроде всё
Настройка VPN VLESS в панели 3X-UI - таких гайдов много. Данный проект призван:
- автоматизировать установку нового
- автоматизировать получение/обновление сертификатов для VLESS-Reality "сертификат берем у себя"
- обновлять geo-файлы, чтобы **трафик до России заворачивать назад**.
- ненапряжный веб-файлообменник для обмена большими файлами (закачка по паролю). 
- ненепряжное статическое хранилище файлов (закачка по ssh)

### Еще можно
Перевесить sshd c 22 порт на повыше, прикрутить бесплатную graphana и мониторить загрузку VPS. Отрубить UDP и ICMP. Порт панели 3X-UI закрыть через файрволл.
...
- SUBSCRIPTION_URL (https//mydomain.de/subs52050#MyHomeVPS), в котором прописаны оба русских сервера. Клиенты будут автопереключаться и обновляться.
   Файл подписки настраивается на каком-то одном сервере, его URL отдается клиентам.

- российские сервера трафик до РФ пускают через себя (direct), до заграницы - через xray reverse заграничных серверов

- ОБЯЗАТЕЛЬНО изменить порт и путь (!) в панели 3X-UI. Панель должна открываться по пути вроде https://mydomain:52050/abc123. Изменения одного порта недостаточно.
