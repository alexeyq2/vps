...
* Создать клиента и указать `flow: xtls-rprx-vision`. Это и есть **VLESS-Reality**.
...
- поднять ненапряжное файлохранилище, чтобы можно было дать прямую ссылку на файлы и скачать без регистрации. 
...
### Рабочий вариант небольшого VPN-узла
...
- URL с подпиской, в которой оба русских сервера. клиенты будут автопереключаться обновляться
  файл подписки настраивается на одном домене и URL отдается клиентам
