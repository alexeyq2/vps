...
2. Отредактовать `.env`, переменные:
  ...
  - `VPS_N`       - рандомный номер, он поменяет
    - `FILEZ_URL`   - статическая веб-шара
...
* `./restart.sh` # пересоздать контейнеры и сразу смотреть логи
...
- непряжное статическое хранилище файлов (закачка по ssh)
